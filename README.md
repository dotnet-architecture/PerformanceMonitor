...
### Data Presentation
The web application is built using ASP.NET CORE and Razor Pages to create a dynamic application. The home page lists all current sessions (with the application name and process name). The user is then prompted to input the specific session that they want to examine. Similar to the other metrics, the index.cshtml.cs page will call the FetchDataService class to get a list of all sessions. This will be explained in more detail later on. Afterwards, another call to the API determines the session that the user is interested in based off the users' input. This session and its ID is used to look up all further data. 
