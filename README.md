...
The HTTP request class contains - in addition to the application name, timestamp, and process - the HTTP event's type (String containing either "Start" or "Stop"), method (String only associated with Starts), path (String only associated with Starts), and activity ID (String - converted from GUID).
